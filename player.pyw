...
def timer_thread(frames_length):
    global current_frame
    timer = fpstimer.FPSTimer(30)
    frames_length -= 1
    while (current_frame < frames_length):
        if is_playing:
            current_frame += 1
        timer.sleep()
    pause()
...
def main(path):#903 586
    ...
    text_box = tk.Text(width = screen_width, height = screen_height, bg = "white", fg = "black", font = ("Consolas", font_size))
    control_panel = ttk.Frame(base)
    pause_but = ttk.Button(control_panel, text="Pause")
    copy_but = ttk.Button(control_panel, text="Copy Frame")
    #rewind_but = ttk.Button(control_panel, text="Restart")
    ...
    #rewind_but['command'] = rewind

    control_panel.pack(side = tk.BOTTOM)
    pause_but.pack(side = tk.LEFT)
    copy_but.pack(side = tk.LEFT)
    #rewind_but.pack(side = tk.LEFT)

    text_box.pack()
    copy_but.pack()
    #rewind_but.pack()

    timer = threading.Thread(target = timer_thread, args = [frames_length])
...
if __name__ == '__main__':
    ...
    main(path = args.path)
#"""
#main('E:\\PythonProjects\\ascii-video\\BadApple.mpascii')
